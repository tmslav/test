...
<script>
	...
	export default {
	  ...
	  watch: {
		...
		{	clearInterval(this.interval)
			if (value>1){
				this.interval=setInterval(this.drawGraph,value*1000-500)
			}
			else{
				this.interval=setInterval(this.drawGraph,1000)
			}
	  ...
	  methods: {
		  ...
		  drawGraph:function(){
			  let uid = this._uid
			  let data = this.getData(this.area)
		  ...
		  getData : function(area){
			      ...
			      var cos = [];
			      for (var i = 0; i < 100; i++) {
					  sin.push({x: i, y: Math.sin(i/10)+Math.random()});
					}
			     return [
					  {
							values: sin,
	...
	created(){
		this.interval = setInterval(this.drawGraph,this.timeOut)
	}
	}
